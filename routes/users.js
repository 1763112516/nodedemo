...
router.post('/reg',function (req, res, next) {
  ...
  pool.getConnection(function (err, connection) {
      ...
      var Password = param.userPassword;
      var _res = res;
      connection.query(userSQL.queryAll, function (err, res) {
          var isTrue = false;
          if(res){ //获取用户列表，循环遍历判断当前用户是否存在
              for (var i=0;i<res.length;i++) {
                  if(res[i].userName == UserName && res[i].userPassword == Password) {
                      isTrue = true;
                  }
              }
          }
...
router.post('/login',function (req, res, next) {
  ...
  pool.getConnection(function (err, connection) {
      ...
      var param = req.query || req.params;
      var UserName = param.userName;
      var userPassword = param.userPassword;
      var _res = res;
      connection.query(userSQL.queryAll, function (err, res, result) {
          var isTrue = false;
          if(res){ //获取用户列表，循环遍历判断当前用户是否存在
              for (var i=0;i<res.length;i++) {
                  if(res[i].userName == UserName && res[i].userPassword == userPassword) {
                      isTrue = true;
                  }
              }
          }
          ...
          if(isTrue) {
              ...
              data.userInfo.userPassword = userPassword;
          } //登录成功返回用户信息
          if(result) {
              result = {
                  code: 200,
